...
def beir_fever_evaluator():
    ...
    evaluator = BeirFEVEREvaluator(pipeline, evaluate_size=5)
...
def test_beir_fever_evaluator(beir_fever_evaluator):
    result = beir_fever_evaluator.evaluate()

    assert len(result.each_results) == 5
    assert result.each_results.iloc[0]['question'] == 'Julie Bowen was born on March 3.'
