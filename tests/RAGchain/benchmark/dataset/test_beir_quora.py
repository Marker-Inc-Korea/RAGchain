...
def beir_quora_evaluator():
    ...
    evaluator = BeirQUORAEvaluator(pipeline, evaluate_size=5)
...
def test_beir_quora_evaluator(beir_quora_evaluator):
    result = beir_quora_evaluator.evaluate()

    assert len(result.each_results) == 5
    assert result.each_results.iloc[0]['question'] == 'Why is the melting point of diamond so much higher than silicon?'
